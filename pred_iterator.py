...
# return the list of all sliced dict
def dict_slices(adict, step):
    keys = adict.keys()
    dict_len = len(keys)
    dict_slices = []
    start = 0
    while start <= dict_len:
        end = start + step
        dict_slice = {}
        for k in keys[start : end]:
            dict_slice[k] = adict[k]
        dict_slices.append(dict_slice)
        start += step
    return dict_slices
...
def test_iterator(args):
    ...
    # now we get the docs, but we have to slice the docs dict to avoid memory error
    dict_slices_ls = dict_slices(docs, args.step)
    print 'Slice into ', len(dict_slices_ls)
    slice_num = 1
    doc_vec_dict = {}
    for dict_slice in dict_slices_ls:
        doc_keys = dict_slice.keys()
        X_docs = []
        for k in doc_keys:
            X_docs.append(vecnorm(doc2vec(dict_slice[k], n_vocab), 'logmax1', 0))
            del docs[k]
        X_docs = np.r_[X_docs]
        doc_codes = ae.predict(X_docs)
        doc_vec_dict_slice = dict(zip(doc_keys, doc_codes.tolist()))
        doc_vec_dict = dict(doc_vec_dict_slice, **doc_vec_dict)
        print 'Finish slice',slice_num
        slice_num += 1
    
    dump_json(doc_vec_dict, args.output)
...
def main():
    ...
    parser.add_argument('-step', '--step', type=int, required=True, help='the size of the sliced dict')
